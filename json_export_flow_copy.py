...
def main():
    ...
    collections_exp = [train_fragments_exp, test_fragments_exp]
    for i in range(len(collections)):

        for elem in collections[i]:
            # Temporal segment if the fragment duration allows it
            segments = int((elem['time'][1] - elem['time'][0]) / (args.frames / FPS)) # 25 fps
            for j in range(min(segments, MAX_SEGMENTS)):
                collections_exp[i].append({
                    'vid': elem['vid'],
                    'vpath': elem['vpath'],
                    'time': [elem['time'][0] + j * (args.frames / FPS), elem['time'][0] + (j+1) * (args.frames / FPS)],
                    'match': elem['match']
                })    
    ...
    for frag in train_fragments:
            class_id = frag['match']
            train_frag_count[class_id] += 1
    
    test_frag_count = [0 for _ in range(len(words))]
    for frag in test_fragments:
            class_id = frag['match']
            test_frag_count[class_id] += 1
