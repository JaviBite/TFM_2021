...
# =============================================================================
    ...
    if len(todas_elipses)==0:      
        ...
        todas_elipses = elipses_finales(canny_output, contornos_unidos, contours_finales, ratio, video_writer1)    
        ...
        todas_elipses2 = elipses_finales(canny_output, contornos_unidos, contours_finales, ratio, video_writer1)                         
        ## si he perdido alguna elipse hago deteccion  08-10-21    
        if not(len(todas_elipses) == len(todas_elipses2)):
            # print('se ha perdido una elipse')
            elipses_perdidas = []
            for i in range(len(todas_elipses)):
                ellip1 = todas_elipses[i]
                for j in range (len(todas_elipses2)):
                    ellip2 = todas_elipses2[j]
            ...
            m_iou = interseccion_vs_union(elipses)        
            contornos_unidos = hausdorff(m_iou, contornos_finales, polys, thh=50.0)
            todas_elipses = elipses_finales(canny_output, contornos_unidos, contours_finales, ratio, []) 
                        
        else:
            todas_elipses = todas_elipses2
