...
def main():
    ...
    CLASSES = "remover,poner (?!olla|sarten|cazo),voltear,^(?!cortar|remover|poner|interaccion|poner|voltear)"
    ...
    while (cap.isOpened() and not end):
        ...
        if sec_timer > 1.0:
            ...
            if inside_action:
                ...
                if len(predictions) > 10:
                    pred = sum(np.array(predictions[-10:])/10)
        ...
        if WRITE_VIDEO:
            ...
            if len(pred_per_sec) > 0:
                ...
                text = "Pred: " + pred_str + " : " + str(class_names)
    ...
    np.savetxt("out_demo_yhat.txt", np.array(pred_per_sec), fmt='%.2f')
    np.savetxt("out_demo_y.txt", np.array(pred_per_sec_y), fmt='%.2f')
